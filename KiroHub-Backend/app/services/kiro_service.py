...
class KiroService:
    ...
    async def _stream_generate_assistant_response_as_openai(
        ...
        *,
        ...
        async with client.stream("POST", url, headers=headers, json=payload, timeout=httpx.Timeout(1200.0, connect=60.0)) as resp:
            ...
            # 添加缓存相关字段（模拟 Anthropic Prompt Caching）
            if cache_tokens > 0:
                if cache_hit:
                    usage_dict["cache_read_input_tokens"] = cache_tokens
                else:
                    usage_dict["cache_creation_input_tokens"] = cache_tokens
    ...
    async def chat_completions_stream(self, user_id: int, request_data: Dict[str, Any]) -> AsyncIterator[bytes]:
        ...
        # 模拟 Anthropic Prompt Caching
        cacheable_items, cache_tokens = extract_cacheable_content(
            request_data.get("messages", []),
            request_data.get("system")
        )
        cache_key = compute_cache_key(cacheable_items) if cacheable_items else ""
        cache_hit = False

        if cache_key:
            cache_redis_key = f"prompt_cache:{cache_key}"
            cache_hit = await self.redis.exists(cache_redis_key)
            if not cache_hit:
                await self.redis.set(cache_redis_key, "1", expire=CACHE_TTL_SECONDS)

        # 移除 cache_control（Kiro 不支持）
        request_data = strip_cache_control(request_data)
